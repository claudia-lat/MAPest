...
for vectOrderIdx = 1 : length(dVectorOrder)
    range_fextMEAS = rangeOfSensorMeasurement(berdy, iDynTree.NET_EXT_WRENCH_SENSOR, dVectorOrder{vectOrderIdx});
...
for accSensIdx = 1 : nrOfLinAccelerometer
    ...
    range_linAccMEAS = rangeOfSensorMeasurement(berdy, iDynTree.ACCELEROMETER_SENSOR, data(accSensIdx).id);
...
for ddqIdx = 1 : nrDofs
    range_ddqMEAS = rangeOfSensorMeasurement(berdy, iDynTree.DOF_ACCELERATION_SENSOR, selectedJoints{ddqIdx});
