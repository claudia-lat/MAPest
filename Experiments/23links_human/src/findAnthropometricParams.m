...
neckHeight     = 0.033* H; % neckHeight     = 0.066 * H;
...
%% Mass and Height consistency
totalMass = 2*footMass + 2*toeMass + 2*lowerLegMass + 2*upperLegMass ...
            + pelvisMass + L5Mass + L3Mass  +T12Mass + T8Mass + neckMass + ...
            headMass + 2*shoulderMass + 2*upperArmMass + 2*foreArmMass + 2*handMass;
if abs(M - totalMass) > 1e-8
    error('Somenthing wrong with the mass computation. Check carefully!');
end
totalHeight = footBeta + lowerLegHeight + upperLegHeight + torsoBeta +...
              neckHeight + headDiameter;
if abs(H - totalHeight) > 1e-8
        error('Somenthing wrong with the height computation. Check carefully!')
end
