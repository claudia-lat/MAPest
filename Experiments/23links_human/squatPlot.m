...
hold on
plot (mu_dgiveny(rangeOfDynamicVariable( berdy, iDynTree.DOF_TORQUE, 'jLeftKnee_roty'),:));
    axis tight;
    grid on;  
ylabel('jointTorques')
leg = legend('jLeftAnkle','jLeftHip','jLeftKnee','Location','southeast');
...
hold on
plot(human_state.q(valueFromName(selectedJoints, 'jLeftKnee_roty'),:)*180/pi);
axis tight;
grid on; 
ylabel('jointAngles')
leg = legend('jLeftAnkle','jLeftHip','jLeftKnee','Location','southeast');
...
hold on
plot (mu_dgiveny(rangeOfDynamicVariable( berdy, iDynTree.DOF_TORQUE, 'jRightKnee_roty'),:));
    axis tight;
    grid on;  
ylabel('jointTorques')
leg = legend('jRightAnkle','jRightHip','jRightKnee','Location','southeast');
                    set(leg,'Interpreter','latex');
                    set(leg,'FontSize',13);
title ('Rotation on y axis');                   
...
hold on
plot(human_state.q(valueFromName(selectedJoints, 'jRightKnee_roty'),:)*180/pi);
axis tight;
grid on; 
ylabel('jointAngles')
leg = legend('jRightAnkle','jRightHip','jRightKnee','Location','southeast');
