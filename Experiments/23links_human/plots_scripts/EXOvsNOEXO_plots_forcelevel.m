...
for subjIdx = 1 : length(subjectID)
    ...
    if group1
        ...
        exo00   = load(fullfile(pathToProcessedData00,'exo_forceLevel.mat'));
        ...
        for jointsIdx = 1 : length(selectedJoints.selectedJoints)
            ...
            for blockIdx = 1 : block.nrOfBlocks
                ...
                hold on;
                % Task 00 --> WITH EXO
                plot2 = plot(exo00.exo_forceLevel(blockIdx).torque(jointsIdx,:),'color',greenAnDycolor,'lineWidth',1.5);
        ...
        exo01   = load(fullfile(pathToProcessedData01,'exo_forceLevel.mat'));
        ...
        for jointsIdx = 1 : length(selectedJoints.selectedJoints)
            ...
            for blockIdx = 1 : block.nrOfBlocks
                subplot (5,1,blockIdx)
                % Task 00 --> NO EXO
                plot1 = plot(noexo00.estimatedVariables.tau(blockIdx).values(jointsIdx,:),'color',orangeAnDycolor,'lineWidth',1.5);
                hold on;
                % Task 01 --> WITH EXO
                plot2 = plot(exo01.exo_forceLevel(blockIdx).torque(jointsIdx,:),greenAnDycolor,'lineWidth''lineWidth',1.5);
                title(sprintf('%s, S%02d, Block %s',selectedJoints.selectedJoints{jointsIdx,1}, ...
                    subjectID(subjIdx), num2str(blockIdx)),'Interpreter','latex');
                ylabel('torque [Nm]');
                if blockIdx == 5
...
if group1
    ...
    for blockIdx = 1 : block.nrOfBlocks
        ...
        len = size(exo00.exo_forceLevel(1).torque,2);
        for i = 1 : len
            comparisonWEvsNE.torqueNorm(blockIdx).complete_exo00(1,i) = norm(exo00.exo_forceLevel(blockIdx).torque(:,i));
            comparisonWEvsNE.torqueNorm(blockIdx).torso_exo00(1,i)    = norm(exo00.exo_forceLevel(blockIdx).torque(torso_range,i));
            comparisonWEvsNE.torqueNorm(blockIdx).rightArm_exo00(1,i) = norm(exo00.exo_forceLevel(blockIdx).torque(rightArm_range,i));
            comparisonWEvsNE.torqueNorm(blockIdx).leftArm_exo00(1,i)  = norm(exo00.exo_forceLevel(blockIdx).torque(leftArm_range,i));
            comparisonWEvsNE.torqueNorm(blockIdx).rightLeg_exo00(1,i) = norm(exo00.exo_forceLevel(blockIdx).torque(rightLeg_range,i));
            comparisonWEvsNE.torqueNorm(blockIdx).leftLeg_exo00(1,i)  = norm(exo00.exo_forceLevel(blockIdx).torque(leftLeg_range,i));
        end
    end
